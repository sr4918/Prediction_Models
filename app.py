...
students = pd.read_csv("intelligent1.csv")
#semesters = pd.read_csv("semesters.csv")
students_train =students[students['id'] < 150]
students_test=students[students['id'] > 149]
...
#def get_new_risk_and_uncertainty(factor1,factor2,factor3,factor4,Result, order, beta_total,num_classes):
 #   columns_student=["factor1","factor2","factor3","factor4","Result"]
    #columns_semester=['order', 'beta_total','num_classes']
  #  student_data = pd.DataFrame([[factor1,factor2,factor3,factor4,Result]], columns=columns_student)
...
#def get_forest_risk_and_uncertainty(factor1,factor2,factor3,factor4,Result,order, beta_total,num_classes):
 #   df = pd.DataFrame([[factor1,factor2,factor3,factor4,Result,order, beta_total,num_classes]], columns=['factor1','factor2','factor3','factor4','factor5','gpa','order','beta_total','num_classes'])
...
#def update_plots(student_value,model_value):
 #   factor1,factor2,factor3,factor4,factor5,gpa=get_student_data(student_value)
    ...
    #if(model_value==1):
     #   risk,certainty=get_new_risk_and_uncertainty(factor1,factor2,factor3,factor4,Result,order,beta_total,num_classes)
    #else:
     #   risk,certainty=get_forest_risk_and_uncertainty(factor1,factor2,factor3,factor4,Result, order,beta_total,num_classes)
        
     
  #  data_student = [{
   #         "type": 'scatterpolar',
    #        "r": [factor1, factor2, factor3, factor4, factor1],
     #       "theta": ['Factor 1','Factor 2','Factor 3', 'Factor 4', 'Factor 1'],
      #      "fill": 'toself'
       #     }]
